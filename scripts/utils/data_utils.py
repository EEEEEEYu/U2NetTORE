...
def process_meta_files(mask_dir:str, tore_dir:str, block_size:int, base_number:int, 
    ...
    tv_tore_readers = ToreSeqReader(tv_tore_meta_files) #{i:ToreSeqReader(f, cache_size=cache_size) for i,f in enumerate(tv_tore_meta_files)}
    tv_mask_readers = ImgSeqReader #{i:ImgSeqReader(f, acc_time=acc_time, step_size=step_size, cache_size=cache_size) for i,f in enumerate(tv_mask_meta_files)}
    ...
    if test_characters is not None:
        test_tore_readers = ToreSeqReader(test_tore_meta_files) #{i:ToreSeqReader(f, cache_size=1) for i,f in enumerate(test_tore_meta_files)}
        test_mask_readers = ImgSeqReader(test_mask_meta_files) #{i:ImgSeqReader(f, cache_size=1) for i,f in enumerate(test_mask_meta_files)}
...
def process_meta_files_accumulated(mask_dir:str, tore_dir:str, block_size:int, base_number:int, 
    ...
    tv_tore_readers = ToreSeqReader(tv_tore_meta_files) #{i:ToreSeqReader(f, cache_size=cache_size) for i,f in enumerate(tv_tore_meta_files)}
    tv_mask_readers = MaskSeqReader(tv_mask_subroots) #{i:MaskSeqReader(f, acc_time=acc_time) for i,f in enumerate(tv_mask_subroots)}
    ...
    if test_characters is not None:
        test_tore_readers = ToreSeqReader(test_tore_meta_files) #{i:ToreSeqReader(f, cache_size=1) for i,f in enumerate(test_tore_meta_files)}
        test_mask_readers = MaskSeqReader(test_mask_subroots) #{i:MaskSeqReader(f, acc_time=acc_time) for i,f in enumerate(test_mask_subroots)}
